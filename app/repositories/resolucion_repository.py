...
class ResolucionRepository(BaseRepository):
    ...
    def create(self, payload: dict, conn=None) -> int:
        fields = [
        ...
        if conn is None:
            with self.db.transaction() as managed_conn:
                return self.create(payload, conn=managed_conn)
    ...
    def add_details(self, resolucion_id: int, details: Iterable[dict], conn=None) -> None:
        if conn is None:
            with self.db.transaction() as managed_conn:
                self.add_details(resolucion_id, details, conn=managed_conn)
                return
